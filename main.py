...
class MyClass(QWidget):
    ...
    def on_btn_addVideo_clicked(self):
        ...
        self.outputDir=os.getcwd()+"/output/"+self.filename.split('.')[0]+"/"
        ...
        if(re.match(r"^.*_processed.mp4",filename)!=None):
            ...
            print(self.liveWidget.size())
            #self.liveWidget.resize(self.stackWidget.width()-2,self.stackWidget.height()-2)
            print(self.liveWidget.size())
            #self.liveWidget.screenDockWidget.setMinimumWidth(int(self.liveWidget.width()/2))
            print(self.liveWidget.screenDockWidget.size())
